...
def transfer_data(transfer_type, use_delete, username, host, key_path, origin_path, destination_path, logger, retries,
                  exclude_list=[]):
        ...
        if any(dta in data for dta in ['shared_config', 'all']):
            if not config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR']:
                logger.writelog("info",
                                "WLS_SHARED_CONFIG_DIR not supplied in replication.properties - shared config not used, will not pull")
            else:
                logger.writelog("info", f"Reading remote config.xml file [{config[DIRECTORIES]['WLS_CONFIG_PATH']}]")
                ssh_client = paramiko.SSHClient()
                ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ssh_client.connect(username=config[PRIMARY]['wls_osuser'], hostname=primary_wls_nodes[0],
                                   key_filename=config[PRIMARY]["wls_ssh_key"])
                ...
                origin_apps_path = root.find("xmlns:app-deployment/[xmlns:name='em']/xmlns:source-path",
                                             namespaces).text
                ...
                destination_apps_path = origin_apps_path.replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                                 config[DIRECTORIES]['STAGE_WLS_SHARED_CONFIG_DIR'])
                ...
                destination_domain_path = origin_domain_path.replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                                     config[DIRECTORIES]['STAGE_WLS_SHARED_CONFIG_DIR'])
                ...
                destination_dp_path = origin_dp_path.replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                             config[DIRECTORIES]['STAGE_WLS_SHARED_CONFIG_DIR'])
    ...
    if any(ins in instance for ins in ['wls', 'all']):
        ...
        if any(dta in data for dta in ['private_config', 'all']):
            for index in range(len(standby_wls_nodes)):
                ssh_client = paramiko.SSHClient()
                ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ssh_client.connect(username=config[STANDBY]['wls_osuser'], hostname=standby_wls_nodes[index],
                                   key_filename=config[STANDBY]["wls_ssh_key"])
                sftp_client = ssh_client.open_sftp()
                logger.writelog("debug",
                                f"Checking destination directory exists: {config[DIRECTORIES]['WLS_PRIVATE_CONFIG_DIR']}")
                try:
                    sftp_client.stat(config[DIRECTORIES]['WLS_PRIVATE_CONFIG_DIR'])
                except IOError as e:
                    if e.errno == errno.ENOENT:
                        stdin, stdout, stderr = ssh_client.exec_command(
                            f"mkdir -p {config[DIRECTORIES]['WLS_PRIVATE_CONFIG_DIR']}")
        ...
        if any(dta in data for dta in ['shared_config', 'all']):
            if not config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR']:
                logger.writelog("info",
                                "WLS_SHARED_CONFIG_DIR not supplied in replication.properties - shared config not used, will not push")
            else:
                local_cfg = config[DIRECTORIES]['WLS_CONFIG_PATH'].replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                                           config[DIRECTORIES][
                ...
                root = cfg_xml.getroot()
                namespaces = {"xmlns": "http://xmlns.oracle.com/weblogic/domain"}
                destination_apps_path = root.find("xmlns:app-deployment/[xmlns:name='em']/xmlns:source-path",
                                                  namespaces).text
                destination_apps_path = pathlib.Path(destination_apps_path).parents[0].as_posix()
                origin_apps_path = destination_apps_path.replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                                 config[DIRECTORIES]['STAGE_WLS_SHARED_CONFIG_DIR'])
                logger.writelog("debug", f"Applications origin path: {origin_apps_path}")
                logger.writelog("debug", f"Applications destination path: {destination_apps_path}")
                destination_domain_path = pathlib.Path(config[DIRECTORIES]['WLS_CONFIG_PATH']).parents[1].as_posix()
                origin_domain_path = destination_domain_path.replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                                     config[DIRECTORIES]['STAGE_WLS_SHARED_CONFIG_DIR'])
                logger.writelog("debug", f"Domain origin path: {origin_domain_path}")
                logger.writelog("debug", f"Domain destination path: {destination_domain_path}")
                destination_dp_path = config[DIRECTORIES]['WLS_DP_DIR']
                origin_dp_path = destination_dp_path.replace(config[DIRECTORIES]['WLS_SHARED_CONFIG_DIR'],
                                                             config[DIRECTORIES]['STAGE_WLS_SHARED_CONFIG_DIR'])
                logger.writelog("debug", f"Deployment plan directory origin path: {origin_dp_path}")
                logger.writelog("debug", f"Deployment plan directory destination path: {destination_dp_path}")
                ...
                ssh_client = paramiko.SSHClient()
                ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                ssh_client.connect(username=config[STANDBY]['wls_osuser'], hostname=standby_wls_nodes[0],
                                   key_filename=config[STANDBY]["wls_ssh_key"])
                sftp_client = ssh_client.open_sftp()
                ...
                for dir in additional_dirs:
                    ...
                    if not os.path.isdir(stage_dir):
                        logger.writelog("error",
                                        f"Additional WLS shared directory [{stage_dir}] missing from staging environment - consider re-running pull")
                        push_successful = False
                        continue
...
def tnsnames(logger, config):
    ...
    ssh_client = paramiko.SSHClient()
    ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh_client.connect(username=config[PREM]['wls_osuser'], hostname=prem_wls_nodes[0],
    ...
    try:
        ...
        logger.writelog("error",
                        f"Failed retrieving tnsnames file [{config[TNS]['TNSNAMES_PATH']}] from on-prem WLS node 1: {repr(e)}")
        return False
    sftp_client.close()
    ssh_client.close()
    ...
    for idx in range(len(oci_wls_nodes)):
        ...
        ssh_client = paramiko.SSHClient()
        ssh_client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh_client.connect(username=config[OCI]['wls_osuser'], hostname=oci_wls_nodes[idx],
                           key_filename=config[OCI]["wls_ssh_key"])
        sftp_client = ssh_client.open_sftp()
...
def run(log_level, action, data=None, instance=None, wls_nodes=None, ohs_nodes=None, **kwargs):
    ...
    if action_successfull:
        ...
        logger.writelog("warn",
                        f"Action [{action}] completed with failures - check log file {LOG_FILE} for further information")
