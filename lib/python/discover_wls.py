...
from topology_discoverer_lift_shift import LiftNShiftTopologyDiscoverer
...
def __generate_remote_report_json(model_context):
    ...
    # Adding Domain and Oracle Home to list of files to backup
    missing_archive_entries.append({
        'sourceFile': infra_constants.DOMAIN_HOME_DIR,
        'path': model_context.get_domain_home(),
        'type': archive_map[discoverer.CUSTOM]
    })
    # discoverer.add_to_model(model.get_model_topology(), infra_constants.DOMAIN_HOME_DIR,
    #                         model_context.get_domain_home())
    # if model_context.is_ssh():
    #     return model_context.get_remote _remote_oracle_home
    # else:
    #     return self._oracle_home
    if model_context.is_remote():
        # discoverer.add_to_model(model.get_model_topology(), infra_constants.ORACLE_HOME_DIR,
        #                         model_context.get_remote_oracle_home())
        archive_entry_path=model_context.get_remote_oracle_home()
    else:
        # discoverer.add_to_model(model.get_model_topology(), infra_constants.ORACLE_HOME_DIR,
        #                         model_context.get_effective_oracle_home())
        archive_entry_path = model_context.get_effective_oracle_home()
    missing_archive_entries.append({
        'sourceFile': infra_constants.ORACLE_HOME_DIR,
