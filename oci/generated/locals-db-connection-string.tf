...
locals {
    ...
    oci_db_connection_string = var.oci_db_connection_string_0
    #    existing_vcn_add_seclist = local.is_oci_db ? var.db_existing_vcn_add_secrule : false
    }

    #  is_oci_db_0                     = trimspace(var.oci_db_dbsystem_id_0) == "" ? false : true
    ...
    is_db_connectin_string_0 = trimspace(var.ds_0) != trimspace(var.oci_db_connection_string_0) ? true : false
    ...
    db_system_options_0            = try(lookup(data.oci_database_db_systems.ocidb_db_systems_0[0].db_systems[0], "db_system_options", []), [])
}
