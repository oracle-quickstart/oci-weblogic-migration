...
#    operator = {}
  ...
  validation {
    condition = alltrue([
    for k, v in var.nsgs :
    contains(["bastion", "int_lb", "pub_lb", "wlsservers", "fss"], k)
    ])
    error_message = format("Invalid NSG keys: %s", jsonencode([for k, v in var.nsgs : k
    if !contains(["bastion", "int_lb", "pub_lb", "wlsservers", "fss"], k)
    ]))
  }
}
...
variable "allow_wlsservers_internet_access" {
  default     = true
  description = "Allow wlsservers nodes to egress to internet. "
  type        = bool
}
...
variable "allow_wlsservers_ssh_access" {
  default     = false
  description = "Whether to allow SSH access to wlsservers nodes."
  type        = bool
}
