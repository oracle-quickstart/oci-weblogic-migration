...
#      instance_params = {
#          lakemigcoherence-1 = {
#            availability_domain = var.use_regional_subnet ? local.ad_names[(0) % length(local.ad_names)] : var.availability_domain
#            compartment_id = var.compartment_id
#            display_name   = "lakemigcoherence-1"
#            shape          = var.instance_shape
#
#            defined_tags  = local.defined_tags
#            freeform_tags = var.tags.freeform_tags
...
#            vnic_display_name = "primaryvnic"
#            assign_public_ip  = var.assign_public_ip
#            hostname_label    = "lakemigcoherence-1"
##            compute_nsg_ids   = length(var.compute_nsg_ids) != 0 ? x == 0 ? var.compute_nsg_ids : [element(var.compute_nsg_ids, 1)] : []
#            compute_nsg_ids = []
#            source_type = "image"
#            source_id   = var.instance_image_id
#            are_legacy_imds_endpoints_disabled = var.disable_legacy_metadata_endpoint
#            fault_domain                       = (length(local.ad_names) == 1 || !var.use_regional_subnet) ? lookup(data.oci_identity_fault_domains.wls_fault_domains.fault_domains[(1) % local.num_fault_domains], "name") : ""
#
#            provisioning_timeout_mins = var.provisioning_timeout_mins
#            metadata = {
#              user_data            = data.template_cloudinit_config.config.rendered
...
#          },
#          lakemigcoherence-2 = {
#            availability_domain = var.use_regional_subnet ? local.ad_names[(1) % length(local.ad_names)] : var.availability_domain
#            compartment_id = var.compartment_id
#            display_name   = "lakemigcoherence-2"
#            shape          = var.instance_shape
#            defined_tags  = local.defined_tags
#            freeform_tags = var.tags.freeform_tags
#            subnet_id         = var.subnet_id
#            vnic_display_name = "primaryvnic"
#            assign_public_ip  = var.assign_public_ip
#            hostname_label    = "lakemigcoherence-2"
##            compute_nsg_ids   = length(var.compute_nsg_ids) != 0 ? x == 0 ? var.compute_nsg_ids : [element(var.compute_nsg_ids, 1)] : []
#            compute_nsg_ids = []
#            source_type = "image"
#            source_id   = var.instance_image_id
...
module "wls-instances" {
  ...
  source = "../instance"
  # This is based on machines discovered.
  # 1. Need domain name
  # 2. Need instance shape mapped to OS discovered.
  # 3. Hostname for each machine name.
  instance_params = {   for index,x in var.wls_machines : x.hostname_label => {

    availability_domain = var.use_regional_subnet ? local.ad_names[(x.availability_domain + local.admin_ad_index) % length(local.ad_names)] : var.availability_domain
...
#    else  IP will be replace with "hostname" + subnet
    ...
    hostname_label    = x.hostname_label
    compute_nsg_ids   = x.compute_nsg_ids  #length(var.compute_nsg_ids) != 0 ? x == 0 ? var.compute_nsg_ids : [element(var.compute_nsg_ids, 1)] : []
...
#      log_level            = var.log_level
      mw_vol_mount_point   = lookup(var.volume_map[0], "volume_mount_point")
      mw_vol_device        = lookup(var.volume_map[0], "device")
      mw_volume            = module.middleware-volume.data_volume_ids[x.availability_domain / var.num_volumes]
      data_vol_mount_point = lookup(var.volume_map[1], "volume_mount_point")
      data_vol_device      = lookup(var.volume_map[1], "device")
      data_volume          = module.data-volume.data_volume_ids[x.availability_domain / var.num_volumes]
...
#      fn_application_id              = var.use_autoscaling ? var.fn_application_id : ""
    ...
    are_legacy_imds_endpoints_disabled = var.disable_legacy_metadata_endpoint
    fault_domain                       = (length(local.ad_names) == 1 || !var.use_regional_subnet) ? lookup(data.oci_identity_fault_domains.wls_fault_domains.fault_domains[(x.availability_domain + 1) % local.num_fault_domains], "name") : ""
