...
variable "wlsserver_image_type" {
  default     = "oke"
  description = "Whether to use a platform, OKE, or custom image for wlsserver nodes by default when unspecified on a pool. When custom is set, the wlsserver_image_id must be specified."
  type        = string
  validation {
    condition     = contains(["custom", "oke", "platform"], var.wlsserver_image_type)
...
variable "wlsserver_image_os" {
  ...
  description = "Default wlsserver image operating system name when wlsserver_image_type = 'oke' or 'platform' and unspecified on a pool."
  type        = string
}

variable "wlsserver_image_os_version" {
  default     = "8"
  description = "Default wlsserver image operating system version when wlsserver_image_type = 'oke' or 'platform' and unspecified on a pool."
  type        = string
}
...
variable "wlsserver_disable_default_cloud_init" {
  default     = false
  description = "Whether to disable the default OKE cloud init and only use the cloud init explicitly passed to the wlsserver pool in 'wlsserver_cloud_init'."
  type        = bool
}
...
variable "agent_config" {
  default     = null
  description = "Default agent_config for self-managed wlsserver pools created with mode: 'instance', 'instance-pool', or 'cluster-network'. See <a href=https://docs.oracle.com/en-us/iaas/api/#/en/iaas/20160918/datatypes/InstanceAgentConfig for more information."
  type = object({
