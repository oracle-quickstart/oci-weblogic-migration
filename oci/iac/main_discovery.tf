...
locals{
  ...
  wls_managed_server_non_unique_ports = flatten([
  ...
  wls_managed_server_ports = distinct(compact(local.wls_managed_server_non_unique_ports))
  ...
  #############################################################################
  wls_dynamic_server_enabled = [for cluster in local.wls_topology["Cluster"] : cluster["ServerTemplate"] if try(cluster["DynamicServer"], false)]
  wls_dynamic_server_ports = flatten([
  ...
  wls_dynamic_server_unique_ports = distinct(compact(local.wls_dynamic_server_ports))


  #############################################################################
  #  #Coherence Port
  #  "CoherenceMemberConfig" : {
  #    "UnicastListenPort" : 8876
  #  },
  #############################################################################
  wls_coherence_ports= [for cluster in local.wls_topology["Cluster"] : cluster["ServerTemplate"] if try(cluster["DynamicServer"], false)]
...
#  oci_instances = [for machines in local.wls_machines : merge(item, {newProp = "XYZ"})]
  ...
  machine_nsgs =  {for k,wls in local.machine_placement: k =>  contains(wls,local.wls_adminserver_name) && length(wls) > 1? concat(module.network-compute-admin-nsg.nsg_id ,module.network-compute-managed-nsg.nsg_id) : ! contains(wls,local.wls_adminserver_name) ? module.network-compute-managed-nsg.nsg_id: module.network-compute-admin-nsg.nsg_id }
  oci_instances = {
      for n, x in local.wls_machines : n => {
        ...
        compute_nsg_ids = local.use_existing_subnets ? local.existing_compute_nsg_ids : local.machine_nsgs[n]
      }
  }
}