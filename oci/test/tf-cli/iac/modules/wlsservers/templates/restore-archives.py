...
def delete_file(archive_on_disk, skip_file=False):
    ...
    try:
        ...
        if skip_file:
            logger.info("<{0}> File {1} skiping file for remove".format(method_name,archive_on_disk))
            return
        else:
...
if __name__ == '__main__':
    ...
    bucket="wlsmig_domain"
    temp_store="/tmp"
    try:
        archive_on_disk=download_file_from_oss(bucket_name=bucket,file_name="wlsmig_machine_1-wlsmig_domain-domain_home.tar.gz",store_path=temp_store)
        restore(archive_on_disk,skip_file=False,change_to_dir="/")
        delete_file(archive_on_disk)
        print("processing next wls archive")
        archive_on_disk=download_file_from_oss(bucket_name=bucket,file_name="wlsmig_machine_1-wlsmig_domain-java_home.tar.gz",store_path=temp_store)
        restore(archive_on_disk,skip_file=False,change_to_dir="/")
        delete_file(archive_on_disk)
        print("processing next wls archive")
        archive_on_disk=download_file_from_oss(bucket_name=bucket,file_name="wlsmig_machine_1-wlsmig_domain-weblogic_home.tar.gz",store_path=temp_store)
        restore(archive_on_disk,skip_file=False,change_to_dir="/")
        delete_file(archive_on_disk)
        print("processing next wls archive")
        archive_on_disk=download_file_from_oss(bucket_name=bucket,file_name="wlsmig_machine_1-wlsmig_domain-custom_dirs.tar.gz",store_path=temp_store,skip_file=True)
        restore(archive_on_disk,skip_file=True,change_to_dir="/")
