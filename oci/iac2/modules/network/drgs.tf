...
resource "oci_core_drg" "wls" {
  count          = length(local.drg_attachments) > 0 ? 1 : 0
  compartment_id = var.compartment_id
  display_name   = "wls-${var.state_id}"
...
resource "oci_core_drg_attachment" "wls" {
  count         = length(local.drg_attachments) > 0 && length(oci_core_drg.wls[*]) > 0 ? 1 : 0
  drg_id        = one(oci_core_drg.wls[*].id)
  display_name  = "drg-wls-${var.state_id}"
  defined_tags  = var.defined_tags
  freeform_tags = var.freeform_tags
