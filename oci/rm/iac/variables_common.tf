...
variable "timezone" {
  default     = "Etc/UTC"
  description = "The preferred timezone for wlsservers, operator, and bastion instances."
  type        = string
}
...
variable "ssh_public_key" {
  default     = null
  description = "The contents of the SSH public key file, optionally base64-encoded. Used to allow login for wlsservers/bastion/operator with corresponding private key."
  type        = string
}

variable "ssh_public_key_path" {
  default     = null
  description = "A path on the local filesystem to the SSH public key. Used to allow login for wlsservers/bastion/operator with corresponding private key."
  type        = string
}