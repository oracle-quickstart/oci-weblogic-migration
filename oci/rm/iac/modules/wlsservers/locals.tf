...
locals {
  ...
  block_volume_domain_mountpath = var.wls_archived_volumes.domain.volume_mount_point
  block_volume_domain_device_id = var.wls_archived_volumes.domain.device
  block_volume_domain_size = min(53,var.wls_archived_volumes.domain.size)
  block_volume_mw_mountpath = var.wls_archived_volumes.middleware.volume_mount_point
  block_volume_mw_device_id = var.wls_archived_volumes.middleware.device
  block_volume_mw_size = min(52,var.wls_archived_volumes.middleware.size)
  block_volume_jdk_mountpath= var.wls_archived_volumes.jdk.volume_mount_point
  block_volume_jdk_device_id= var.wls_archived_volumes.jdk.device
  block_volume_jdk_size = min(51,var.wls_archived_volumes.jdk.size)


  #TODO (joi) remove local below when launch_volume_attachments works for all version of OL
