...
cd "${domain_home}/config/jdbc" || (echo "Failed to cd to ${domain_home}/config/jdbc" | log | tee -a $log_file >> $error_log_file ; exit 1)
...
%{ for config_key, jdbc_string in datasources }
  ...
  if [[ "$is_admin_instance" == "true" ]] && [[ "${jdbc_string.existing_vcn_add_seclist}" == "true" ]]; then
     ...
     echo "$output" | log >> $log_file
     if [ $exit_code -ne 0 ]; then
        echo "Error executing the script to open ingress port ${jdbc_string.db_port} in db subnet ${jdbc_string.db_subnet_id}" | log | tee -a $log_file >> $error_log_file
        echo "$output" | log >> $error_log_file
        exit 1
     fi
  fi
  ...
  if [ $is_atp == "true" ]; then
     ...
     if [ -n "$matching_files" ]; then
        ...
        echo "$output" | log >> $log_file
        if [ $exit_code -ne 0 ]; then
          echo "Error executing datasource update for DB System database on jps-config*.xml files.. Exiting provisioning" | log | tee -a $log_file >> $error_log_file
          echo "$output" | log >> $log_file
          exit 1
        fi
     else
     ...
     if [ -n "$matching_files" ]; then
        ...
        echo "$output" | log >> $log_file
        if [ $exit_code -ne 0 ]; then
           echo "Error executing datasource update for custom JDBC connection string on jps-config*.xml files.. Exiting provisioning" | log | tee -a $log_file >> $error_log_file
           echo "$output" | log >> $error_log_file
           exit 1
        fi
     else
