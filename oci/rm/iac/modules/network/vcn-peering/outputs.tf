...
output "dblpg_ids" {
  ...
  value = {
    ...
    k => v.is_vcn_peering ? try(oci_core_local_peering_gateway.dblpg_0[k].id, null) : null
  }
}
...
output "wlslpg_ids" {
  ...
  value = {
    for k, v in var.datasources :
    k => v.is_vcn_peering ? try(oci_core_local_peering_gateway.wlslpg_0[k].id, null) : null
  }
}