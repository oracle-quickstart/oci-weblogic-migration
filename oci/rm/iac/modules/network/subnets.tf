...
locals {
  ...
  subnet_info = {
    ...
    wlsservers  = { create = false, is_public = var.wlsserver_is_public }
    fss      = { create = contains(keys(var.subnets), "fss") }
    int_lb = {
      create         = contains(["both", "internal"], var.load_balancers),
      create_seclist = true, dns_label = "ilb",
    }
    pub_lb = {
      create         = contains(["both", "public"], var.load_balancers),
  ...
  # otherwise use "<subnet_key><state_id>" as the default DNS label to ensure uniqueness.
  ...
  subnet_output = { for k, v in var.subnets :
    k => lookup(v, "id", null) != null ? v.id : lookup(lookup(oci_core_subnet.wls, k, {}), "id", null)
  }
}
...
resource "oci_core_subnet" "wls" {
  ...
  security_list_ids          = compact([lookup(lookup(oci_core_security_list.wls, each.key, {}), "id", null)])
...
resource "oci_core_security_list" "wls" {
  for_each = {
...
output "wlsserver_subnet_id" {
  value = lookup(local.subnet_output, "wlsservers", null)
}
output "wlsserver_subnet_cidr" {
  value = contains(keys(local.subnet_output), "wlsservers") ? lookup(local.subnet_cidrs_all, "wlsservers", null) : null
}
