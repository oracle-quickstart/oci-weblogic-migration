...
locals {
  ...
  int_lb_nsg_enabled = var.add_load_balancer && anytrue([
  ...
  int_lb_rules = local.int_lb_nsg_enabled ? merge(
    {
      "Allow TCP egress from internal load balancers to wlsservers for Node Ports" : {
      ...
      "Allow ICMP egress from internal load balancers to wlsserver nodes for path discovery" : {
      ...
      "Allow TCP egress from internal load balancers to wlsservers for health checks" : {
...
resource "oci_core_network_security_group" "int_lb" {
  ...
  display_name   = "${var.resource_name_prefix}-int_lb-${var.state_id}"
  vcn_id         = var.vcn_id
