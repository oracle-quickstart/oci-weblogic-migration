...
locals {
  ...
  create_iam_wlsserver_policy = anytrue([
    ...
    var.create_iam_wlsserver_policy == "auto" #&& var.create_cluster && anytrue([for k, v in var.wlsserver_pools :
#    tobool(lookup(v, "create", true)) &&
#    lookup(v, "mode", var.wlsserver_pool_mode) != "node-pool"
#      ])
  ])

  create_iam_autoscaler_policy = anytrue([
    var.create_iam_autoscaler_policy == "always",
    var.create_iam_autoscaler_policy == "auto" #&& var.create_cluster && anytrue([for k, v in var.wlsserver_pools :
#    tobool(lookup(v, "create", true)) &&
