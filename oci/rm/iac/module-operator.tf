...
locals {
  ...
  operator_private_ip = (local.create_domain_enabled && var.create_operator
...
module "operator" {
  ...
  defined_tags = merge(var.use_defined_tags ? {
    ...
    "${var.tag_namespace}.domain"     = local.wls_domain_name,
  } : {}, local.operator_defined_tags)
  freeform_tags = merge(var.use_defined_tags ? {} : {
    "state_id" = local.state_id,
    "role"     = "operator",
    "domain"   = local.wls_domain_name
