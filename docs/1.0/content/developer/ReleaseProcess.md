---
title: "OWM project release process"
date: 2019-02-23T17:19:24-05:00
draft: false
weight: 5
---

This document describes the process that should be followed to create a OCI Weblogic Migration (OWM) release.

### Prerequisites
- A local installation of WebLogic Server 12.2.1.x must be available.
- The person running the release process must have admin privileges on the [OCI Weblogic Migration GitHub repo](https://github.com/oracle/oci-weblogic-migration) because the release process pushes to the master branch directly.
...
1. Set (and export) the following environment variables:
    ...
    - `OWM_SCM_REPO_URL` - set to the browsable URL to the project (e.g., `https://github.com/oracle/oci-weblogic-migration`)
    - `OWM_SCM_REPO_CONN` - set to the clonable URL for the project (e.g., `git@github.com:oracle/oci-weblogic-migration.git`)

2. In the `oci-weblogic-migration` project directory, run the `mvn -B release:prepare release:perform` command.
...
Note that this process relies on the OWM installers being in your local Maven repository.  As such, it is critical for
the same user to run these steps on the same machine as the steps from the previous section!

1. Save the release notes in the file `<OWM-project-directory>/target/ReleaseNotes.md`.
...
3. Log into [GitHub](https://github.com/oracle/oci-weblogic-migration), go to the Releases page, review/edit the draft release, and then publish the release.
