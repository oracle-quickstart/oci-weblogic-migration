...
This OWM domain model sample has a typical configuration for a configured cluster with a single managed server, including connection information, logging setup, and other details.
...
```bash
$ ${OWM_HOME}/bin/modelHelp.sh -oracle_home /tmp/oracle topology:/Server
```
