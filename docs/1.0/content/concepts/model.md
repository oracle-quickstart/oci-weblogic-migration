...
- [Upgrading from OWM 1.x to OWM 2.x YAML files](#upgrading-from-OWM-1x-to-OWM-2x-yaml-files)
...
- `verrazzano`     - The location where the Verrazzano WebLogic Workload component and application configuration is specified. **NOTE**: Verrazzano support is deprecated in OWM 4.0.0.
...
Second, notice that the `jsf#2.0` shared library `SourcePath` attribute value starts with `@@WL_HOME@@`. This is a path
...
The OCI Weblogic Migration handles names of WebLogic Server configuration artifacts in a very prescribed way.  To
...
In the example above, the model has two instances of `JDBCSystemResource`: one named `MyDataSource` and one named
...
meaning.  As such, the OCI Weblogic Migration does not expose these names in the model, as shown below:
...
`/SecurityConfiguration/<domain-name>`.  The OCI Weblogic Migration has built-in rules and a knowledge base that
...
used to change the name.  As with the previous class of artifact, the OCI Weblogic Migration does not support the use
...
**Variable tokens** are declared with the syntax `@@PROP:<variable>@@`. This type of token represents a value that is
...
The root directories are configured as a comma-separated list of directories, using the environment variable
`OWM_MODEL_SECRETS_DIRS`. For example, if `OWM_MODEL_SECRETS_DIRS` is set to `/etc/my-secrets,/etc/your-secrets`,
...
`OWM_MODEL_SECRETS_NAME_DIR_PAIRS` to map `<name>` values to specific directory locations. For example, if
`OWM_MODEL_SECRETS_NAME_DIR_PAIRS` is set to `my-root=/etc/my-secrets,your-root=/etc/your-secrets`, then the token
`@@SECRET:your-root:the-secret@@` will look for the secrets file at:
```
/etc/your-secrets/the-secret
```
If the `<name>` value has a corresponding mapped directory in `OWM_MODEL_SECRETS_NAME_DIR_PAIRS`, then that directory
will take precedence over any roots specified in `OWM_MODEL_SECRETS_DIRS`.
...
When modeling configuration attributes that can have multiple values, the OCI Weblogic Migration tries to make this
...
One of the primary goals of the OCI Weblogic Migration is to support a sparse model where the user can specify just
...
With OCI Weblogic Migration, you can specify named items in the model to be deleted using the Create Domain,
...
are prepended with an exclamation point (!) in the model. **NOTE**: The Deploy Applications Tool is deprecated in OWM 4.0.0.
...
A named element using [delete notation](#declaring-named-mbeans-to-delete) will completely delete an element with a
...
### Upgrading from OWM 1.x to current OWM version YAML files

Beginning in 2.0, OCI Weblogic Migration now incorporates the SnakeYAML parser for reading and writing model files.
