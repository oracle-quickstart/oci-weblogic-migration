...
Starting in OWM 4.0, some types previously stored under the top-level `wlsdeploy` directory have been moved to the
...
provisioning them with OWM are:
...
file or a set of one or more files.  If it is a ZIP file, that ZIP file will be expanded in place when running OWM
...
in place when running OWM tools like the Create Domain or Update Domain tools.
...
`$DOMAIN_HOME/management-services-ext/` directory. **NOTE**: The Deploy Applications Tool is deprecated in OWM 4.0.0.
