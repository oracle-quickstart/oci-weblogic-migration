...
In OWM 4.2.0, several optional features have been added to change the default behavior that make it possible
...
- online mode - Online mode uses WLST online to read the domain configuration using the Administration Server's MBeans.
  ...
  - SSH mode - In this mode, the tool can be run from any host for which you have set up SSH connectivity between the
    host where OWM is running and the host where the Administration Server is running.  The `-ssh_user` (which, by
...
To run the Discover Domain Tool in online mode, you have several options as described earlier.  In normal mode, OWM
...
By default, the Discover Domain Tool, like all other OWM tools, assumes that the domain is a pure WebLogic Server domain.
...
OWM has introduced features that allow you to more fully discover a domain by discovering passwords stored in the domain
...
command-line.  This feature works in both offline and online modes provided that OWM has access to the domain home
directory (that is, it does not work with the online `-remote` argument).  By default, OWM will try to use 
OWM encryption to encrypt the passwords in the model using the OWM passphrase passed to the tool, which requires using
...
{{% notice warning %}}OWM provides a configuration setting in `tool.properties` that allows the passwords to be stored
in the model without being encrypted.  Oracle strongly recommends that you do not use this feature since not only will
it result in passwords being stored in clear text in the model but also may result in passwords in the OWM log files.
{{% /notice %}}
...
The OWM model file already has model sections for each of these providers' data, as follows:
...
except for users, because OWM needs to discover the users' passwords to use when creating a new domain from the
discovered model.
{{% /notice %}}

When discovering the `DefaultAuthenticator` or the `DefaultCredentialMapper`, any discovered passwords will be stored
in the model using OWM encryption; therefore, the OWM encryption passphrase must be provided using one of the
...
When discovering the `XACMLAuthorizer` or the `XACMLRoleMapper`, OWM analyzes the XACML documents to try to determine
...
A OWM model used to create a domain require that the default administrator user name and password be provided in the
...
`store.discover.admin_credentials` property in the OWM tool.property file
...
| `-admin_pass_env`              | An alternative to entering the admin password at a prompt. The value is an environment variable name that OWM will use to retrieve the password.                                                 |                                            |
...
| `-passphrase_env`              | An alternative to entering the encryption passphrase at a prompt when discovering passwords. The value is an environment variable name that OWM will use to retrieve the passphrase.             |                                            |
| `-passphrase_file`             | An alternative to entering the encryption passphrase at a prompt when discovering passwords. The value is the name of a file with a string value which OWM will read to retrieve the passphrase. |                                            |
| `-passphrase_prompt`           | Allow OWM to prompt for the encryption passphrase or read it from stdin.                                                                                                                         |                                            |
