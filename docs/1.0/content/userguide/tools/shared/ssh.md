...
draft: false
weight: 2
description: "Describes OWM tooling SSH support."
---

OWM 4.0 introduces the ability to perform online operations remotely using SSH to provide access to the remote
...
### SSH prerequisites 
OWM does not attempt to configure SSH between the machines.  As such, you must ensure that SSH is working between
the machines in question and have made an initial SSH connection outside OWM as the user that OWM will use.  This
...
### SSH verification
The Verify SSH Tool will allow the user to verify that SSH is working properly for use by OWM.  For example, to verify
