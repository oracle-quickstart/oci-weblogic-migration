...
from the host where OWM is running to the host where the domain's Administration Server is running.  It also assumes that
...
If the model or variables file contains passwords encrypted with the OWM Encrypt Model Tool, you must provide the
OWM encryption passphrase used to encrypt the model so that the Create Domain Tool can decrypt them.  To do this,
...
| `-passphrase_env`                     | An alternative to entering the encryption passphrase at a prompt. The value is an environment variable name that OWM will use to retrieve the passphrase.                                                                                                                                             |                                            |
| `-passphrase_file`                    | An alternative to entering the encryption passphrase at a prompt. The value is a the name of a file with a string value which OWM will read to retrieve the passphrase.                                                                                                                               |                                            |
