...
The OCI Weblogic Migration has a built-in logging framework based on `java.util.logging`.  Its logging configuration
is specified in `$OWM_HOME/etc/logging.properties`.  By default, the logging framework writes to both the console and
a log file.

#### Log file
By default, OWM tools write their log files to the `$OWM_HOME/logs` directory and the log file name reflects the name of the tool.
For example, if you run the `validateModel` tool then the log file will be `$OWM_HOME/logs/validateModel.log`.  These
log files are overwritten each time you run a particular tool so the file contains only the logs from the last tool
invocation.

If the `$OWM_HOME/logs` directory is not writable by the user running the tool, the logging framework will search for
...
- Check the temp directory (as defined by the `java.io.tmpdir` Java system property) and create a `OWM-logs` subdirectory.

If none of these locations are writable, the logging framework prints an error message to `stderr` and exits.

#### Console output
OWM tools output logging information to `stdout` and `stderr`, as appropriate.  By default, only `INFO` level messages
...
#### Logging levels
As mentioned previously, OWM's logging framework is based on `java.util.logging` so all logging levels defined in
the `java.utiul.logging.Level` class apply to OWM loggers.  For a quick review of those levels, see the
[javadoc](https://docs.oracle.com/javase/8/docs/api/java/util/logging/Level.html).

OWM uses hierarchical loggers that align with the purpose of the code being executed.  The root logger is named `wlsdeploy`.
Many loggers exist underneath the root logger; for example, `wlsdeploy.create`, `wlsdeploy.discover`, and `wlsdeploy.util`.
By default, the OWM `logging.properties` file sets the logging level of the root and several important loggers.  If the
...
In OWM, the log file will collect log entries from all loggers based on the logger's `level` while the console output
is limited to `INFO` and above only.  Log entries written to the console will not display any exception stack traces
associated with a log entry.  To see those, you must look at the log file.  The OWM logging framework supports using the
...
#### Log handlers
OWM uses several log handlers to handle logging output of data to various sources.

 | Log Handler | Output Destination | Description                                                                      |
 | ----------- |----------------------------------------------------------------------------------| ----------- |
 | `java.util.logging.FileHandler` | OWM tool log file | The standard `java.util.logging` file handler.                                   |
 | `oracle.weblogic.deploy.logging.StdoutHandler` | `stdout` | OWM handler that writes `INFO` level messages to the console.                    |
 | `oracle.weblogic.deploy.logging.StderrHandler` | `stderr` | OWM handler that writes `WARNING` and `SEVERE` level messages to the console.    |
 | `oracle.weblogic.deploy.tooling.SummaryHandler` | `stdout` | OWM handler that writes the tool's execution summary information to the console. |
...
Any attempts to set other configuration for these log handlers will simply be discarded by the OWM logging framework at startup.
