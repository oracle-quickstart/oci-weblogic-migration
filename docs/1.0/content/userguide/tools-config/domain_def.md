...
---
 ...
 OCI Weblogic Migration has an extensible domain type system.  The three built-in domain types (`WLS`, `RestrictedJRF`,
 ...
 [Custom configuration]({{< relref "/userguide/tools-config/custom_config.md" >}}) directory `$OWM_CUSTOM_CONFIG/typedefs`.
...
```
 OCI Weblogic Migration provides the `JRF-Compact.json` type definition file that can be used to create a JRF domain
 ...
 target resources to the correct clusters and servers. The default behavior for OCI Weblogic Migration is to invoke
